...
use app\wechat\exception\WeChatException;
...
class UserToken extends Token
{
    ...
    public function get()
    {
        $result = Curl::curlGet($this->wxLoginUrl);

        // 注意json_decode的第一个参数true, 这将使字符串被转化为数组而非对象
        $wxResult = json_decode($result, true);
        if (empty($wxResult)) {
            // 为什么以empty判断是否错误，这是根据微信返回, 规则摸索出来的
            // 这种情况通常是由于传入不合法的code
            throw new Exception(['message' => '获取session_key及openID时异常，微信内部错误',]);
        } else {
            // 建议用明确的变量来表示是否成功
